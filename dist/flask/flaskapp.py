...
def nor(aditz):
    ...
    hand = open("/home/aditzaksoftware/app/nor.json", "r")
    info = hand.read()
    NOR = loads(info)
...
def nornork(aditz):
    hand = open("/home/aditzaksoftware/app/nk3.json", "r")
    info = hand.read()
    NK3 = loads(info)
...
def page():
    if request.method == 'GET':
        ...
        input_str = request.form['aditza-input']
        output = analyse(input_str)
        if input_str != "" and not " " in input_str:
            return render_template("base.html", Aditza=output["Aditza"], Kasua=output["Kasua"], Modua=output["Modua"], Denbora=output["Denbora"], Nor=output["Nor"], Nori=output["Nori"], Nork=output["Nork"])

        if output["Kasua"] == "NOR":
            if output["Nor"] != None and output["Nori"] == None and output["Nork"] == None:
                eel.out(dumps(x))
            else:
                flash("Aditz hau ez da existitzen.")

        if output["Kasua"] == "NOR-NORI":
            if output ["Nor"] != None and output["Nori"] != None and output["Nork"] == None:
                eel.out(dumps(x))
            else:
                flash("Aditz hau ez da existitzen.")

        if output["Kasua"] == "NOR-NORK":
            if output ["Nor"] != None and output ["Nori"] == None and output ["Nork"] != None:
                eel.out(dumps(x))
            else:
                flash("Aditz hau ez da existitzen.")
